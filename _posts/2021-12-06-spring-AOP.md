...
기존의 OOP를 이용하여 문제를 해결할 수는 있으나 아름다운 코드라고는 할 수 없겠죠.
...
`@Aspect` 애너테이션을 처리하기 위해서는 AspectJ Auto Proxy를 활성화해야해요.
