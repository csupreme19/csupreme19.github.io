...
PostgreSQL max_connections 설정 방법(Ubuntu 기준으로 작성)

---
## 시스템 재부팅 방법

시스템을 장애 없이 Graceful 하게 shutdown하는 방법을 다룬다.
...
### Kubernetes 서버
Gracefully shutdown kubernetes nodes

> [https://stackoverflow.com/a/59576322/15263734](https://stackoverflow.com/a/59576322/15263734)
...
```
해당 노드에서 실행되는 파드를 모두 제거하고 해당 노드의 파드 스케줄링을 비활성화한다.
...
- 파드 상태 `Succeeded` 정상종료 확인
  ...
  > `drain` 명령어는 파드의 정상 종료(Graceful shutdown)를 보장한다
...
> 기존 떠있는 Pod들은 Reschedule 시점까지 새로 생긴 Node로 스케줄 되지 않음
...
$ ps -ef
```
systemd 시스템 데몬에 등록된 프로세스들은 리부트시 자동으로 실행된다.

systemd를 사용하지 않는 프로세스는 OS 재시작시 재기동 여부 확인 후 재기동되지 않으면 수동 구동 필요

systemd에 등록이 되어 자동 실행된다고 하더라도 실제 작동 여부 확인 필요하다.

### 서비스별 systemd 사용 여부

아래 테이블은 오픈소스 리스트와 systemd 등록 여부 및 수동 시작 명령어를 작성하였다.

설치형을 기준으로 작성되었으며 버전 및 환경별로 다를 수 있으니 참고만 할 것
...
구동되어 있지 않으면 `systemctl start 서비스`로 정상 실행여부 확인한다

그래도 실행되지 않으면 위 표에 있는 수동 시작 명령어 실행(권장하지 않음)
