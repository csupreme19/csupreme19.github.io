...
시간이 지날수록 코드가 더 좋아지는 프로젝트를 만들 수 있다.

코드의 퇴보를 막고 지속적인 개선을 통하여 코드 품질을 유지할 수 있음
...
예외 처리를 사용하면 오류 처리 코드가 원래 코드에서 분리된다.
...
테스트케이스는 **예제로 보여주는 문서**이다.

잘 만든 테스트 케이스는 그 자체로 훌륭한 문서 역할을 한다.
...
따라서 CPU, 메모리 자원 관리에 효율적인 구현보다 읽기 쉬운 테스트 코드를 작성하는 것이 옳다.
...
변수나 함수에 접근은 private로 접근을 기본적으로 공개하지 않는 편이 좋다.
...
위 코드의 경우 함수의 메서드 수는 적절할지 몰라도 클래스의 책임이 너무 많다.

1. 버전 정보에 대한 책임
2. 컴포넌트에 대한 책임

위 두가지에 해당하는 책임을 가지고 있으므로 단일 책임 원칙에 위배된다고 할 수 있다.
...
5분마다 변경되는 StockExchange 구현체가 있다고 가정해보자
...
단일 책임 원칙에 따라 의존성을 생성하는 책임은 다른 곳에 전가한다.

클래스는 완전히 수동적인 존재로 의존성을 해결하지 않으며 의존성을 주입 받아 사용.

스프링 프레임워크가 가장 널리 알려진 자바 DI 컨테이너를 핵심 기능으로 제공한다.
...
위 코드에서는 초기화 시점에 모두 생성하는 것이 아닌 사용직전에만 인스턴스를 생성한다.

이를 **초기화 지연(Lazy Initialization)**이라고 함

DI 개념을 써도 초기화 지연 사용 가능

대부분의 DI 컨테이너에서 팩토리, 프록시, 레이지 로딩등을 지원한다.

<br>

### 확장

작은 마을에 6차선 도로를 깔지는 않는다.

작은 마을 -> 도시 -> 큰 도시로 성장하며 확장이 일어남
...
따라서 시스템은 반복적이고 점진적인 발전이 필요 => 확장성 고려

이를 성취하기 위해 애자일, TDD를 도입하고 리팩터링을 사용하는 것

물리적인 시스템과 달리 소프트웨어 시스템은 관심사를 적절히 분리하여 확장성을 가져갈 수 있음

<br>

### 횡단 관심사

로깅, 트랜잭션 등의 관심사를 소스 코드가 아닌 외부에 구성하는 것

소스 코드에서 부가 기능(로깅, 트랜잭션, 예외 등)을 횡단 분리(cross-cutting)하는 것

#### 관점 지향 프로그래밍(Aspect-Oriented Programming)

횡단 관심사를 모듈화로 확보하는 방법론으로 모듈 구성 개념은 관점(Aspect)이다.
...
자바 프록시를 사용하기 위해서는 인터페이스와 구현체를 정의하고 InvocationHandler를 구현해야한다.

코드양이 많으며 복잡하기 때문에 깨끗한 코드를 작성하기 어렵다.
...
POJO는 어떠한 의존성도 가지고 있지 않기 때문에 단순하며 간편하다.

스프링은 빈과 같은 정보를 XML 파일에 명시하여 사용한다.

XML은 읽기 어렵다는 단점에도 프록시 보다는 간단하다.

<br>

### Test Driven 아키텍처 구축

관점으로 관심사(도메인)를 분리(낮은 결합)

애플리케이션을 POJO로 작성한다면 코드 수준에서 관심사가 분리된다.

소프트웨어는 물리적인 형체가 없으므로 관점 분리를 통해 효과적인 변경이 가능

=> 테스트 주도 아키텍처 설계가 가능하다!

건축설계: BDUF(Big Design Up Front)

> 구현 전에 가능한 모든 것을 설계하는 것

소프트웨어: 단순하면서 확장성있고 가용성이 있는 결과물을 재빨리 출시하여 점진적 확장

<br>

### 표준이 항상 정답은 아니다

기존 EJB2는 표준이라는 이유만으로 많이 사용됨

가벼운 설계로도 충분한 프로젝트도 EJB2를 채택하는 경우가 존재 => 고객 가치가 뒤로 밀려난다

<br>

### 결론

POJO를 사용하여 관점을 분리하고 의도를 명확히 표현하여 기민성을 높여야함.

시스템은 실제로 돌아가는 가장 단순한 방법을 사용해야한다.
...
##### 테스트 케이스를 만들고 계속 테스트하라

위 규칙을 따르면 자동적으로 객체지향 방법론이 지향하는 낮은 결합도와 높은 응집력을 달성할 수 있다.
...
위 1번의 테스트 케이스를 모두 작성했다면 코드를 정리할 수 있다.

1. 코드를 추가할때마다 설계가 제대로 되었는지 점검한다.
2. 점검을 통해 코드를 깔끔하게 정리한다.
3. 테스트 케이스를 돌려 기존 기능에 영향이 없는지 확인한다.
4. 위 방법을 점진적으로 반복한다.

**테스트 케이스가 있으니까** 시스템에 버그가 발생하거나 잘못되는 경우는 없다
...
위 코드는 메서드간 중복코드가 발생하였음

아래와 같이 수정하면 중복을 제거한다.
...
코드 작성자는 작성 시점에 자신의 코드에 푹 빠져 있어 이해하기 쉽다.

하지만 코드를 유지보수하거나 보는 사람이 깊이 있게 이해하기란 불가능

> 하지만 나중에 코드를 읽을 사람은 바로 자기 자신임을 명심
...
클래스마다 인터페이스를 무조건 생성하라거나 자료 클래스와 동작 클래스를 무조건 분리해야한다거나하는 

정책은 득보다 실이 많은 경우가 있음
...
작업 처리량(Throughput)을 개선하기 위하여, 처리 속도를 개선하기 위하여 필요
...
- 직관적이며 구현이 간편하다 
- 눈에 잘 보인다
  - 디버깅할때 호출 스택만으로 상태가 드러남

**멀티 스레드 프로그램**

- 비직관적이며 구현이 어렵다
- 눈에 잘 안보인다
  - 거대한 루프문 하나인 프로그램이 아닌 작은 여러개의 프로그램으로 보인다
...
1. **동시성은 항상 성능을 높여준다**
   ...
   - 동시성은 특수한 상황에서 성능을 높여준다.

   - 여러 스레드가 프로세스를 공유하거나 처리할 독립적인 계산이 많은 경우에만 성능이 높아진다. 

   - 일반적인 상황에서 항상 성능이 좋아지는 것은 아님.

2. **동시성을 구현해도 설계는 변하지 않는다**

   - 싱글 스레드 시스템과 멀티 스레드 시스템은 설계가 완전 다르다.

3. **웹 컨테이너를 사용하면 동시성을 이해할 필요가 없다**

   - 컨테이너 동작 원리, 동시 수정, 데드락과 같은 문제를 이해해야만 한다.
...
1. **동시성은 부하를 유발한다**
   ...
   - 성능 부하가 존재하며 코드도 더 짜야함

2. **동시성은 복잡하다**
   - 간단한 문제라도 동시성을 구현하려면 매우 복잡해진다.
   
3. **동시성 버그는 재현이 어렵다**
   - 동시성 버그는 재현이 어려워 진짜 버그가 아니라 일회성 버그라고 여기기 쉽다.
...
**동시성 코드는 다른 코드와 분리할 것**

동시성 코드는 동시성이라는 단일 책임을 가지고 있으므로 다른 코드와 분리가 필요하다

ex) 동기 코드와 비동기 코드를 동시에 쓸 수 없다.
...
위 예시에서 보았듯 여러 스레드가 동일 객체를 공유한다면 스레드 간섭으로 잘못된 결과가 나온다.

공유 객체의 코드 내 임계 영역(Critical Section)을 줄이고 synchronized 등의 키워드로 보호 필요
...
>
> 라이브락은 프로세스가 살아있는채로 유의미한 진행이 안되는 것
...
생산자 스레드가 정보를 생성해 버퍼나 큐 등의 대기열에 넣으면 소비자 스레드가 대기열에서 정보를 가져다 사용
...
철학자 왼쪽에는 포크가 있고 스파게티를 먹으려면 양손에 포크를 쥐어야함

포크가 사용중이라면 포크를 내려놓을 때까지 대기

해당 문제는 실제로 여러 기업 애플리케이션에서 겪는 문제로 설계가 잘못되면 데드락, 라이브락, 처리 효율 저하등의 문제 발생
...
동기화하는 메서드 사이에 의존성이 존재하면 동시성으로 찾아내기 힘든 버그 발생

<br>

### 동기화 부분 최소화

**임계 영역을 최소화하여 동기화를 최소화하라**

자바에서는 `synchronized` 키워드로 공유 자원에 대한 락을 설정할 수 있다.

락이 설정된 임계 영역에서는 하나의 스레드만 접근 가능하다.

락 설정은 스레드를 지연시키고 부하를 발생시키므로 개수를 최소화 하는 것이 좋다.

> 그렇다고 임계 영역의 개수를 줄이는 대신 영역을 거대하게 키우면 스레드 경쟁으로 인해 성능이 떨어진다.

<br>

### 올바른 종료 코드 작성

**종료 코드를 초기부터 고민하고 동작하도록 시간을 투자하라**

스레드를 모두 깔끔하게 종료하기란 어렵다
...
수천분의 일의 확률로 말이 안되는 실패가 일어난 경우 대개 시스템 오류로 취급한다.

이런 실패는 아주 가끔씩 발견되어 단순 일회성 버그로 치부하고 넘어가는 경우 매우 위험하다.

잠정적인 동시성 스레드 문제로 고려할 것

#### 스레드 환경과 스레드 환경 밖의 테스트 분리

스레드 환경 밖에서 테스트가 정상인지 확인할 필요가 있다.

#### 다양한 환경을 지원하도록 구현

단일 스레드, 여러 스레드, 실행 중 스레드 바꾸기 등을 지원하도록 테스트 케이스를 작성할 것

#### 다른 플랫폼에서 돌려보기

윈도우와 맥에서의 스레드 정책은 다르다.
...
- 공유 코드 임계 영역을 이해
- 아주 때때로 발생하는 일회성 오류를 넘기지 마라
- TDD를 적용하여 테스트를 쉽게 할 것

---

## 17. 냄새(Code Smells)와 휴리스틱

이 장에서는 대표적인 코드 냄새와 휴리스틱에 대해서 나열식으로 기술한다.

> **휴리스틱(Heuristics)**
>
> 시간이나 정보의 부족으로 합리적인 판단이 불가능할때 어림짐작하는 발견법

위의 장에서 설명한 내용과 중복되는 부분이 많으나 정리하는 차원에서 한번 쭉 읽어볼 것
...
5. **주석 처리된 코드**
   - 주석 처리 코드는 즉각 지워라 => 소스 코드 버전 관리 시스템이 기억한다.
...
1. **빌드가 여러 단계이다**
   - 빌드는 하나의 명령으로 끝나야한다.
   - 부가적인 스크립트나 시스템 의존성을 가져오는 등의 추가 단계가 없어야함
2. **테스트가 여러 단계이다**
   - 모든 단위 테스트는 단 한번의 클릭(명령)으로 수행 되어야 함
...
1. **너무 많은 인수**
   - 함수의 인수는 없는 것이 가장 좋음
   - 있는 경우 인수를 최소화
   - 인수가 넷 이상인 경우 함수 재설계를 고려
2. **출력 인수**
   - 사용자 직관에 위배된다 => 인수는 항상 입력이어야 한다.
   - 함수의 상태를 변경해야 한다면 함수가 속한 객체의 상태를 변경하도록 바꾸기
3. **플래그 인수**
   - boolean 인수는 함수가 여러 기능을 수행한다는 뜻
   - 단일 책임 원칙에 위배되므로 플래그 인수 사용하지 않을 것
4. **죽은 함수**
   - 아무도 호출 하지 않는 함수는 가차없이 지운다. => 역시 소스 코드 관리 시스템이 기억한다.
...
1. **한 소스 파일에 여러 언어 사용**
   - 자바 소스 파일에 HTML, YAML, JavaScript 등을 함께 포함하지 않는다.
2. **당연한 동작 구현하지 않음**
   - 당연한 동작도 구현을 해야 저자를 신뢰하고 함수 기능을 직관적으로 이해한다.
3. **경계를 올바르게 처리하지 않음**
   - 모든 경계 조건을 찾아내고 테스트하는 테스트 케이스를 작성하라
4. **안전 절차 무시**
   - 컴파일러, IDE의 경고를 꺼버리면 빌드가 쉬워지나 끝없는 디버깅에 시달릴 수도 있음
5. **중복**
   - **이 책에 나오는 가장 중요한 규칙**
   ...
   - 어디서든 중복을 발생하면 없애라
   - 최근 15년동안 나온 디자인 패턴들은 대다수가 중복을 제거하는 잘 알려진 방법에 불과함
6. **잘못된 추상화 수준**
   - 추상화 인터페이스는 세부 구현에 관련된 상수, 변수, 유틸리티를 몰라야 한다.
7. **기초 클래스가 파생 클래스에 의존**
   - 기초 클래스가 파생 클래스에 의존하면 독립적으로 배포가 불가능하다.
8. **과도한 정보**
...
10. **수직 분리**
    - 변수와 함수는 사용되는 위치에 가깝게 선언
    - 비공개 함수는 처음 호출 직후에 정의
11. **일관성 부족**
    - 특정 개념을 일관성 있게 동일한 방법으로 구현한다.
12. **잡동사니**
    - 사용되지 않는 변수, 함수, 기본 생성자 등을 지워라
13. **인위적 결합**
    - 함수, 상수, 변수 선언시에 시간을 들여 올바른 위치에 넣으려 고민하라
14. **기능 욕심**
    - 어떤 클래스가 다른 클래스의 변수와 함수에 관심을 가져서는 안된다.
15. **선택자 인수**
    - 인수를 넘겨서 해당 인수로 함수의 동작을 분기하거나 제어하는 경우 => 새로운 함수로 분리
16. **모호한 의도**
    - 헝가리식 표기법, 매직 넘버, 행을 바꾸지 않은 수식 등
17. **잘못 지운 책임**
    - 코드를 배치하는 위치는 독자가 자연스럽게 기대하는 위치에 배치
18. **부적절한 static 함수**
    - 인스턴스 함수를 static 함수로 정의하는 실수
19. **서술적 변수**
    - 서술적인 변수 이름을 쓰면 가독성이 늘어남
20. **이름과 기능이 일치하는 함수**
    - 이름이 애매모호하면 문서를 찾아보지 않는 이상 기능을 추론하기 어렵다
21. **알고리즘을 이해하라**
    - 알고리즘을 충분히 이해하고 코드를 작성하라
    - 테스트케이스를 모두 통과하는 것으로는 부족하다.
22. **논리적 의존성은 물리적으로 드러내라**
    - 예시) 55페이지 크기를 처리할 수 있는 A라는 클래스가 B 클래스를 사용한다면 B 클래스 역시 55페이지 크기를 처리할 수 있어야 한다.
    - B 클래스에 getMaxpPageSize()와 같은 메서드로 페이지 크기를 가져온다면 논리적 의존성이 물리적 의존성으로 변함 
23. **if/else, switch/case문보다 다형성을 사용하라**
    - switch문 대신 다형성 객체를 생성하여 사용하라
24. **표준 표기법을 따르라**
    - 팀은 업계 표준에 기반하여 팀 표준을 설정하고 그 표준은 팀원 모두가 따라야 한다.
25. **매직 넘버는 명명된 상수로 변경하라**
    - 숫자는 명명된 상수 뒤로 숨겨라.
    - SECONDS_PER_DAY=86400
26. **정확하라**
    - 통화를 다뤄야한다면 부동 소수점을 쓰지 말고 정수로
    - null을 반환한다면 항상 null체크
    - 동시성 코드를 작성한다면 적절한 락
27. **관례보다 구조를 사용하라**
    - switch/case 문보다  추상 메서드 + 기초 클래스
28. **조건을 캡슐화하라**
    - boolean 논리는 생각보다 어렵다.
    - shouldBeDeleted 등의 변수명으로 확실한 의도를 밝혀라
29. **부정 조건은 피하라**
    - if(!shouldBeDeleted(obj)) 보다 if(shouldNotBeDeleted(obj))가 더 직관적으로 이해하기 쉬움
30. **함수는 한 가지만 해라**
    - 중첩 구조가 생길 정도로 길면 안되고 함수는 한 가지만 수행해야 한다.
    - 여러 가지를 수행하는 한 가지 함수 => 한 가지를 수행하는 여러 함수 
31. **숨겨진 시간적 결합**
    - 함수 호출 실행 순서를 보장하라
32. **일관성 유지**
    - 코드 구조가 일관적이면 남들도 그 일관성을 따른다.
33. **경계 조건 캡슐화**
    - 경계 조건은 코드 여기 저기서 처리하지 않는다.
    - 경계 조건을 별도 변수로 캡슐화하여 사용(ex: int nextLevel = level + 1)
34. **함수는 추상화 수준을 한 단계만 내려가야 함**
    - 함수에서 추상화 수준을 분리한다.
35. **설정 정보는 최상위 단계에 둬라**
    - 기본 상수나 설정 관련 상수는 함수에 숨겨선 안되고 최상위 단계에 둔다.
36. **추이적 탐색을 피하라**
    - 디미터 코드 법칙을 준수한다.
    - myClass.doSomething()과 같이 단순해야 한다.
...
1. **긴 import 목록을 피하고 와일드 카드를 사용하라**
   ...
   - 명시적 import는 실제 그 클래스가 반드시 존재해야 하지만 와일드카드 형태는 검색 경로를 추가하는 형식으로 진정한 의존성이 생기지 않음
2. **상수는 상속하지 않는다.**
   - 상수를 상속하면 계층에 가려져 가독성이 저해된다.
   - 꼭 필요하다면 import static을 활용한다.
3. **상수 vs Enum**
   - public static final int 보다 enum 열거체를 사용하라!
   - 훨씬 유연하고 서술적이며 편하다.
...
1. **서술적인 이름을 사용한다.**
   ...
   - 코드만 보고 어떤 기능을 하는 함수인지 어떤 값을 담고 있는 변수인지 혹은 상수인지 판단 가능
2. **적절한 추상화 수준에서 이름 짓는다.**
   - 구현을 드러내는 이름은 피하라.
   - 인터페이스에 추상화 수준이 너무 낮은(너무 구현/구체적인) 이름은 피한다.
3. **가능하다면 표준 명명법 사용**
   - 데코레이터 패턴의 경우 클래스명에 Decorator 사용 등
4. **명확한 이름**
   - 길지만 서술적인 함수명, 변수명을 사용하면 주석이 필요 없음
5. **긴 범위는 긴 이름을 사용**
   - 범위(스코프)가 커지면 서술적인 이름을 사용하고 지역변수와 같이 스코프가 작아지면 아주 짧은 이름을 사용한다.
6. **인코딩을 피하라**
   - 헝가리안 표기법을 사용하지 말라
   - 접두어로 자료형, 범위 같은 정보를 넣어서는 안된다.
7. **이름으로 부수 효과를 설명**
   - 함수명에 분기 처리되는 부수 효과에 대한 내용도 이름에 작성한다.
...
1. **불충분한 테스트**
   - 테스트는 잠재적인 버그가 발생할 만한 부분을 모두 커버해야한다.
2. **커버리지 도구를 사용**
   - 커버리지 도구는 테스트가 빠뜨리는 부분을 알려준다.
   - 대다수 IDE에서는 테스트 커버리지를 시각적으로 표현해준다.
3. **사소한 테스트를 생략하지 마라**
   - 사소한 테스트는 오히려 짜기 쉽다.
   - 사소한 테스트의 가치는 실제 구현 코드보다 높다.
4. **무시한 테스트는 그 자체로 모호함을 뜻한다**
   - 불분명한 요구사항의 경우 @Ignore나 테스트 케이스를 주석처리하게 되는데 컴파일이 가능한 경우 테스트 진행한다.
5. **경계 조건 테스트**
   - 특이 케이스와 같은 경계 조건을 모두 테스트 한다.
6. **버그 주변은 철저히 테스트**
   - 한 함수에서 버그가 발생하면 그 주변에서 버그가 발생할 확률이 높다.
7. **실패 패턴을 확인**
   - 꼼꼼한 테스트 케이스를 작성했다면 실패하는 테스트 케이스의 패턴만 보고 문제를 진단할 수 있음
   - ex) 음수 값이 인수로 들어가는 케이스 모두 실패, 입력값이 5글자를 넘어가면 모두 실패 등
8. **테스트 커버리지 패턴을 확인**
   - 테스트 커버리지 패턴으로 실패 원인 파악 가능
9. **테스트는 빨라야 한다**
   - 느린 테스트 케이스는 실행을 꺼리는 경향이 있어 테스트를 누락할 가능성 높음
