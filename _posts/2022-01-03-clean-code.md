...
시스템 구현: 당장 주어진 요구사항과 유저 스토리에 맞추어 시스템을 구현
...
동시성은 결합을 없앤다 => what과 when을 분리한다.

싱글 스레드 프로그램: what과 when이 밀접하다 => 디버깅할때 호출 스택만으로 상태가 드러남

멀티 스레드 프로그램: 거대한 루프문 하나인 프로그램이 아닌 작은 여러개의 프로그램으로 보인다. 

무엇과 언제를 분리하면 앱 구조와 효율이 향상됨
