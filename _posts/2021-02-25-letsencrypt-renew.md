...
본 문서는 무료 SSL 인증서를 제공하는 Let's Encrypt를 사용하여 인증서 발급 및 갱신을 하는 방법을 정리했어요.
...
인증서 발급 방법은 수동으로 certbot 설치 후 발급 진행 하는 방법과 docker image로 발급하는 방법이 있어요.

---
### docker image로 인증서 발급 방법
docker volume path의 mount 지점에 인증서가 생성돼요.
```sh
...
- certonly 옵션으로 인증서만 발급 → 웹 서비스의 설정 파일 자동으로 변경하지 않아요.
- standalone 방식으로 발급 시 web serivce를 잠시 중단
- 하루 동안 3번만 발급 가능하므로 발급 시 주의가 필요
...
DNS TXT 값 표시되는 화면에서 엔터 눌러 진행하지 않고  **대기** 후 아래 도메인 설정 등을 진행해야 해요.
...
도메인 호스팅 서비스의 DNS 관리메뉴로 진입하여 dns txt record 값을 설정해야 해요.

예를 들어 가비아의 경우 아래 그림과 같이 'DNS 레코드 추가' 메뉴에서 TXT 레코드를 추가 가능해요.

![lr-2.png]({{ "/assets/img/contents/lr-2.png"}})

호스트 컬럼에는 **_acme-challenge** 만 입력하고

가비아의 경우 설정한 값 예시는 아래 이미지 참고하고 record 값 앞/뒤로 쌍따옴표 필요해요.

![lr-3.png]({{ "/assets/img/contents/lr-3.png"}})

해당 record 설정 후 저장하여 반영하여 아래 command로 도메인 서버에 레코드가 반영 되었는지 확인 가능해요.
...
정상 반영 시 아래 이미지와 같이 확인 가능하고

![lr-4.png]({{ "/assets/img/contents/lr-4.png"}})

이후 앞 과정의 dns 방식 인증서 발급 대기 화면에서 enter를 눌러 진행해요.
...
Let's Encrypt의 경우 무료이지만 인증서 유효 기간이 3개월로 짧아서

인증서 만료 전 반드시 갱신을 해야만 site에 접속이 안되는 현상을 방지할 수 있어요.

3개월 주기로 알람을 등록하여 인증서 갱신 관리는 하는 것을 추천해요.

또한 인증서를 manual 로 생성했을 경우 certbot 자동 갱신 명령어로는 갱신 되지 않아서 아래 명령어로 갱신가능해요.

```sh
$ certbot --server https://acme-v02.api.letsencrypt.org/directory -d "*.yourdomain.com" --manual --preferred-challenges dns-01 certonly
```

명령어 입력 후 나오는 DNS TXT record 값을 도메인 호스팅 사이트에서 DNS 설정에 들어가 업데이트 후 진행해요.

> 인증서를 갱신하여 새로운 인증서를 받아도 기존 인증서는 만료되지 않는 점 참고하세요.

---
### 인증서 갱신 후 서비스별 적용 가이드

기본적으로 NAS에 인증서 정보를 저장하고 각 서비스에 가져다 사용해요.

본 예제에서는 인증서를 `/nas/volumes/ssl`에서 관리하였고

- k8s의 경우 kubernetes secret 형태로 떠져 있는 형태
- 그 외 docker로 직접 올라가있는 경우 각 서비스별 인증서 정보 적용

위에서 발급받은 인증서파일을 관리하는 인증서 폴더로 옮겨야해요.
...
인증서 갱신 및 발급 후에 인증서 적용이 성공적으로 되었으나 `NET::ERR_CERT_COMMON_NAME_INVALID`로 인증서가 유효하지 않다고 나오는 경우가 있었어요.

#### 인증서의 주소와 인증서가 적용된 도메인의 주소가 일치하지 않을 때 발생하는 오류

1. 인증서의 도메인 주소가 제대로 적용되었는지 확인
2. `*` wildcard가 포함되어 있는지 확인
3. 도메인 주소가 정상인 경우 전세계의 DNS 서버에 적용이 바로 되지 않고 시간이 어느정도 소요되어서 발생하는 문제로 기다리면 해결돼요.
...
`Authoritative answers can be found from:`에 리스트로 도메인 서버 정보가 나오면 등록이 완료된 것이라고 보시면 돼요.
