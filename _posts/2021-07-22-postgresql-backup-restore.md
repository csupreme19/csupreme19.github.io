...
Postgres 백업 방식은 [Continuous Archiving](https://www.postgresql.org/docs/14/continuous-archiving.html) 방식으로 두 가지의 파일을 백업하고 있어요.

> [PostgreSQL WAL 아카이브 백업]({% post_url 2021-06-26-postgresql-archive-backup %}) 참고하세요.

1. `basebackup`

   postgresql data 파일 자체 백업으로
   
   이후 WAL 파일을 이용하여 복구시 기준이 되는 postgres 파일이에요.

2. `pg_wal`

   postgres WAL(write ahead log) 미리쓰기 파일로 수행된 모든 쿼리를 담고 있어요.

### 백업 방법

1. pg_wal 자동 archiving 설정

`postgresql.conf` 설정파일 수정이 필요해요.
...
`archive_command` 명령어는 아카이빙 시점에 실행되는 커맨드로 0을 리턴해야해요.

이후 postgres restart가 필요해요.

> config reload 수행하여 설정파일을 reload하는 방식은 모든 셋팅에 적용되는 건 아니며<br>위의 `wal_level`, `archive_mode` 설정의 경우는 restart가 필요해요.
...
만약 `archive_command`만 수정되었다면 postgres restart 필요 없이 아래 쿼리로 설정 reload 가능해요.
...
백업의 기준이 되는 basebackup 파일은 `pg_basebackup` 명령어로 수동 생성하여야 해요.
...
백업 output 폴더는 비어있거나 존재하지 않아야하며 db user(`postgres`) permission이 있어야 해요.
...
1. 서버 중지
   ...
   shutdown 종류에 관한 설명은 [Postgresql shutdown]({% post_url 2021-08-06-postgresql-shutdown-mode %}) 참고하세요.
...
4. 기존에 백업한 data 백업 파일 복구(복사)
   ...
   db user(`postgres`)로 permission이 있어야해요.
   ```shell
...
6. data 하위 `recovery.conf` 생성
   ...
   ```
   `recovery_target_time`은 복구 시점(해당 시점으로 DB가 복구)
   
   `recovery_target` 미지정시 마지막 WAL 레코드까지 복구 수행해요.
   
   자세한 내용은 [Recovery Target Settings](https://www.postgresql.org/docs/11/recovery-target-settings.html) 참고하세요.
7. 서버 시작시 `recovery.conf` 파일을 감지하고 복구 모드에 돌입하며
   
   복구 완료시 `recovery.conf`를 `recovery.done`으로 파일명 자동 변경돼요.
8. 복구 완료되었는지 체크
   1. `recovery.conf` 파일명이 `recovery.done`으로 변경되어 있는지 확인
   
      `recovery.conf` 파일이 존재하면 Postgresql이 복구 모드에 돌입하여 모든 transaction이 read-only 상태예요.
   2. DB 쿼리 조회하여 데이터가 해당 시점으로 복구가 되었는지 확인
   
> 복구시 사용 가능한 WAL segment들을 사용하므로 마지막에는 `file not found` 메세지가 나오는데 이는 정상이에요.

---

## 복구(Postgresql 12 이상)

Postgresql 12 이전 버전에서는`recovery.conf`를 사용하지만 12버전부터는 `postgresql.conf`와 `recovery.signal를 사용해요.

> 설치형이 아닌 Docker Container 환경이라고 가정할게요.
...
6. data 하위 `postgresql.conf` 수정
   ...
   ```
   `recovery_target_time`은 복구를 원하는 시간(해당 시점으로 DB가 복구)
...
7. `recovery.signal` 생성
   ...
   ```
7. 서버 시작시 `recovery.signal` 파일을 감지하고 복구 모드에 돌입하며

   복구 완료시 `recovery.signal`을 자동 삭제해요.
8. 복구 완료되었는지 체크
   1. `recovery.signal` 파일이 지워졌는지 확인, 안지워졌으면 지워줘요.
   
      `recovery.signal` 파일이 존재하면 Postgresql이 복구 모드에 돌입하여 모든 transaction이 read-only 상태예요.
   ...
   3. DB 쿼리 조회하여 데이터가 해당 시점으로 복구가 되었는지 확인해요.
...
basebackup 수행 이후로 max_connections 설정이 늘어난 경우로 `postgresql.conf`의 max_connections를 해당 수치로 수정하면 해결돼요.
...
복구 이후 아카이브 되는 시점에 문제가 발생하는 경우로 복구 이후에 모종의 이유로 꼬였을 때 나는 오류로

충돌이 발생하는 파일을 archivedir에서 제거하여 해결했어요.(pg_wal에 있는 WAL이 다시 복사되어 문제 없어요.)
